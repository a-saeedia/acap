...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [prevPath, setPrevPath] = useState('/app/assets')
  ...
  useEffect(() => { if (pathname !== '/app') setPrevPath(pathname) }, [pathname])
    ...
    <div className="min-h-screen bg-gray-950 text-white flex" dir="rtl">
      ...
      {!sidebarOpen && (
        <button onClick={() => setSidebarOpen(true)}
          className="hidden md:flex fixed right-0 top-1/2 -translate-y-1/2 z-30 p-2 rounded-r-xl bg-gray-900/95 border border-gray-800 border-l-0 text-gray-400 hover:text-white hover:bg-gray-800 transition-all"
        >
      ...
      <main className={`flex-1 pt-16 md:pt-0 min-h-screen transition-all duration-300 ${sidebarOpen ? 'md:mr-64' : 'md:mr-0'}`}>
        <div className="max-w-6xl mx-auto p-4 md:p-8">
          {pathname !== prevPath && (
            <button onClick={() => router.push(prevPath)}
