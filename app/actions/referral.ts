...
// Invite reward milestones (from ambassador-section.tsx)
...
export async function applyReferralCode(code: string) {
  ...
  const referrer = await db.select().from(userProfile).where(eq(userProfile.referralCode, code.toUpperCase())).limit(1)
  ...
  await db.update(userProfile).set({ referredBy: code.toUpperCase() }).where(eq(userProfile.userId, session.user.id))
...
export async function markReferralConverted(referralId: string) {
  ...
  for (const m of INVITE_MILESTONES) {
    if (convertedCount === m.invites) {
      // Grant the reward (e.g., extend subscription)
      ...
      // Mark milestone on the latest referral
...
export async function checkAndGrantMilestones() {
  const session = await auth.api.getSession({ headers: await headers() })
  if (!session?.user) throw new Error('Unauthorized')
  const refs = await db.select().from(referral).where(
    and(eq(referral.referrerId, session.user.id), eq(referral.status, 'active'))
  )
  const granted = refs.filter(r => r.rewardMilestone).map(r => r.rewardMilestone)
  const totalInvites = refs.length
  const nextMilestone = INVITE_MILESTONES.find(m => !granted.includes(m.action) && totalInvites >= m.invites)
